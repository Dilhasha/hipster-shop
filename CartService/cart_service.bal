...
service /cartservice on new http:Listener(9233) {
    ...
    resource function post updateCart(int userId, int productId, int quantity) {
        io:println("Cart updated for user id:" + userId.toString() + " product id: " + productId.toString() + " quantity: " + quantity.toString());
    }
}
...
function getProductData(int productId) returns json {
    ...
    return {"productId val": productId, "productName": "MacBook Pro", "price": 2000.00};
}
